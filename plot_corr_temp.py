...
print("Loading regridded data...")

# === LOAD DATA ===
era5_MPI = xr.open_dataset(data_path / "t2m_ERA5_1981_2010_MPI.nc").squeeze()
era5_FIO = xr.open_dataset(data_path / "t2m_ERA5_1981_2010_FIO.nc").squeeze()
mpi  = xr.open_dataset(data_path / "tas_1981-2010.nc").squeeze()
fio  = xr.open_dataset(data_path / "tas_FIO-ESM-1981-2010.nc").squeeze()

era5_MPI_temp = era5_MPI.t2m - 273.15
era5_FIO_temp = era5_FIO.t2m - 273.15
...
def compute_spatial_corr(da_obs, da_model, chunk_size=50):
    ...
    for lat_start in range(0, da_obs.shape[1], chunk_size):
        ...
        obs_chunk = da_obs.isel(lat=slice(lat_start, lat_end)).values
        model_chunk = da_model.isel(lat=slice(lat_start, lat_end)).values
...
mpi_corr_field = compute_spatial_corr(era5_MPI_temp, mpi_temp, chunk_size=50)

print("Computing FIO correlations...")
fio_corr_field = compute_spatial_corr(era5_FIO_temp, fio_temp, chunk_size=50)
...
im1 = Global_plot(mpi_corr_field, 'a) MPI-ESM1-2-HR', axs[0], levels_corr)
im2 = Global_plot(fio_corr_field, 'b) FIO-ESM-2-0', axs[1], levels_corr)
...
plt.tight_layout()
plt.savefig('./Plots/02_correlation_MPI_FIO_vs_ERA5.png', dpi=300, bbox_inches='tight')
plt.show()
